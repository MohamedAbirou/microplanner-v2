...
| 5 UX completeness | IN PROGRESS | Full browser click-through done 2026-07-06 → **[docs/PHASE5_BROWSER_QA.md](PHASE5_BROWSER_QA.md)**. Every route in §4 visited via real headed browser + real Clerk session + real Stripe test checkout (FREE→STARTER). **Found P0:3 / P1:6 / P2:8 / P3:6.** P0s: (1) Tasks/Week/Month crash on any goal-less task (`task.goal.color` null deref), (2) Sign Out button has no handler, (3) logged-out `/` funnel bounces to sign-in (`/home` missing from middleware public routes). Core loop VERIFIED end-to-end (goal→generate→accept→materialize→analytics) + billing lifecycle (checkout/portal/cancel/resume) solid. Fix pass NOT yet done — do NOT mark DONE until P0s cleared. audit-contracts + audit-operations both green. |
