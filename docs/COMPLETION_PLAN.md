...
| 4 Money & retention (partial) | PARTIAL | DONE: notifications context wired to real GraphQL (was mock); analytics page real data (was demo); plan-ready/booking/reminder emails honor NotificationPreferences; integrations credentials AES-256-GCM encrypted; OAuth exchanges implemented for 6 providers; webhook retry/backoff; waitlist admin gate (ADMIN_EMAILS). REMAINING: Stripe test-mode checkout→webhook→tier walk (endpoints exist; needs valid Stripe test keys + stripe CLI listen) |
| 5 UX completeness | NOT STARTED | browser click-through; command palette; empty states; settings persistence. Zero TODO/FIXME markers repo-wide (apps/*/src); tier-limit warning restored in plan wizard |
| 6 Hardening | NOT STARTED | tests, README-for-buyer, env example accuracy, branch cleanup, tag v1.0.0. NOTE: .env/.env.local contain live-looking test keys — rotate + gitignore before sale |
