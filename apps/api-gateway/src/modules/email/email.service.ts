...
export class EmailService {
  ...
  /**
   * Send waitlist invitation email (user has been approved to join)
   * Uses inline HTML so a missing template file can never break invitations.
   */
  async sendWaitlistInvitation(email: string, name: string): Promise<void> {
    if (!this.isEnabled) {
      this.logger.debug('Email service disabled, skipping waitlist invitation');
      return;
    }

    try {
      const signUpUrl = `${this.appUrl}/sign-up?invited=1&email=${encodeURIComponent(email)}`;
      const html = `
        <div style="font-family: -apple-system, Segoe UI, Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 24px;">
          <h1 style="font-size: 22px;">You're in, ${name || 'there'}! 🎉</h1>
          <p>Your spot on the MicroPlanner waitlist has been approved. You can now create your account and start planning your week with AI.</p>
          <p style="margin: 28px 0;">
            <a href="${signUpUrl}" style="background: #6d28d9; color: #fff; padding: 12px 24px; border-radius: 8px; text-decoration: none;">Create your account</a>
          </p>
          <p style="color: #666; font-size: 13px;">If the button doesn't work, copy this link into your browser:<br/>${signUpUrl}</p>
        </div>`;

      const response = await this.resend.emails.send({
        from: this.fromEmail,
        to: email,
        subject: "🎉 You're invited to MicroPlanner!",
        html,
      });

      if (response.error) {
        this.logger.error(`Failed to send waitlist invitation email: ${JSON.stringify(response.error)}`);
        return;
      }

      this.logger.log(`Waitlist invitation email sent to ${email} - Email ID: ${response.data?.id}`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to send waitlist invitation email: ${errorMessage}`);
      // Don't throw - email failures shouldn't break the invite flow
    }
  }
