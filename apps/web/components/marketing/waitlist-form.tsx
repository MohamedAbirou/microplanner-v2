  ...
  if (isSuccess) {
    return (
      <div className="flex items-center justify-center p-6 bg-gradient-to-r from-primary-600/20 to-accent-700/20 border border-primary-600/30 rounded-md animate-fade-in">
        ...
        <div>
          <p className="text-lg font-semibold text-dark-text-primary">You're on the list!</p>
  ...
  if (variant === 'hero') {
    return (
      <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-2xl mx-auto">
        ...
        {errorMessage && <p className="mt-3 text-sm text-error text-center">{errorMessage}</p>}
        <p className="mt-4 text-sm text-dark-text-tertiary text-center">
          Join <span className="text-primary-500 font-semibold">1,234</span> people waiting. Get{' '}
          <span className="text-accent-500 font-semibold">3 months PRO free</span> as an early
          adopter.
        </p>
      </form>
    );
